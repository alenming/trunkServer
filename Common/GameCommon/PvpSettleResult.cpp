...
int CPvpSettleResult::calcIntegral(int myIntegral, int otherIntegral, int result)
{
	...
	return _PSRMAX(_PSRMIN(integralChange, 50), -50);
}
...
void CPvpSettleResult::setWinnerPvpModel(int roomType, int uid, int changeMMR, int changeInte, int newRank)
{
	...
	if (roomType == PVPROOMTYPE_PVP)
	{
        ...
        bool isNewHighestIntegral = mapValues[PVP_FD_HISTORYHIGHESTINTEGRAL] < mapValues[PVP_FD_INTEGRAL];
		//历史最高积分
        if (isNewHighestIntegral)
		{
        ...
        // 升段位处理
        int oldDan = CCommonHelper::getUserIntegralDan(mapValues[PVP_FD_INTEGRAL] - changeInte);
        int newDan = CCommonHelper::getUserIntegralDan(mapValues[PVP_FD_INTEGRAL]);
        if (oldDan < newDan)
        {
            integralDanUp(uid, oldDan, newDan, isNewHighestIntegral);
        }
	}
...
void CPvpSettleResult::setLoserPvpModel(int roomType, int uid, int changeMMR, int changeInte, int newRank)
{
	...
	if (roomType == PVPROOMTYPE_PVP)
	{
		...
		//竞技积分, 此时公式算出的changeInte为负数,如果小于750不扣分
        if (changeInte < 0)
        {
            if (mapValues[PVP_FD_INTEGRAL] >= PVP_INTEGRAL_BOUNDARY)
            {
                mapValues[PVP_FD_INTEGRAL] += changeInte;
...
void CPvpSettleResult::overFairPvp(int winner, int loser, int result, int winerCryLv, int loserCryLv, int dt)
{
	...
	if (winnerDropId > 0)
	{
		...
		CPropUseHelper::getInstance()->AddItems(winner, vecDropItems, true, PVPDROP_GAIN_REASON);
	}
...
void CPvpSettleResult::integralDanUp(int uid, int oldDan, int newDan, bool isFirst)
{
    // 如果是首次从新手升到青铜，奖励3宝箱
    if (isFirst 
        && newDan == DAN_BRONZE 
        && oldDan == DAN_NEWBIE)
    {
        ...
        int lastGenTime = 0;
        model->GetPvpField(PVPCOMM_FD_LASTCHESTGENTIME, lastGenTime);

        if (lastGenTime > 0)
        {
            model->SetPvpField(PVPCOMM_FD_LASTCHESTGENTIME, lastGenTime - 3 * pChestSetting->ChestRefreshTime);
        }
    }
}
